...
APIRET t_ThreadRead::ThreadReadBlock (t_pFifoBlock &pFifoBlock)
{
               ...
               switch (RcSeek)                          // the source device, let's replace it by a zero sector.
               {
                  case NO_ERROR:
                     ...
                     if ( CONFIG(BadSectorLogThreshold) == 0                          ||  // Threshold feature disabled if cfg param is zero
                        ...
                        LOG_INFO ("[%s] Read error on sector %Ld, bad data replaced by zero block (%Ld bad sectors up until now)", QSTR_TO_PSZ(pDevice->LinuxDevice), Sector, BadSectorCount)
                     if (BadSectorCount == (quint64)CONFIG(BadSectorLogThreshold))
                        LOG_INFO ("[%s] Bad sector threshold has been reached, only logging every 1000th bad sector from now on.", QSTR_TO_PSZ(pDevice->LinuxDevice), Sector)
   ...
   } while ((BytesRead < BytesToRead) && (pDevice->GetState() != t_Device::AcquirePaused)
                                      && (pDevice->GetState() != t_Device::VerifyPaused )
                                      && !pDevice->Error.Abort());

   if ((pDevice->GetState() == t_Device::AcquirePaused) ||
       (pDevice->GetState() == t_Device::VerifyPaused ))
   {
      ...
      *pBlocks = 0;
      ...
      do
      {
         ...
         switch (rc)
         {
            ...
            case ERROR_THREADREAD_DEVICE_DISCONNECTED:
               while (!pDevice->Error.Abort() && ((pDevice->GetState() == t_Device::AcquirePaused) ||
                                                  (pDevice->GetState() == t_Device::VerifyPaused )) )
